<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:GeneralAffairslist:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:GeneralAffairslist:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:GeneralAffairslist:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:GeneralAffairslist:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="GeneralAffairslistList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="主键" align="center" prop="generalaffairslistid" />
...
<script>
import { listGeneralAffairslist, getGeneralAffairslist, delGeneralAffairslist, addGeneralAffairslist, updateGeneralAffairslist, exportGeneralAffairslist }
from "@/api/goodpass/GeneralAffairslist";

import { listEmployee21 } from "@/api/system/Employee2";


export default {
  name: "GeneralAffairslist",
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      total: 0,
      total4:0,
      // GeneralAffairs签核人员表格数据
      GeneralAffairslistList: [],
      // 弹出层标题
      title: "",
      // 是否显示弹出层
      open: false,
  ...
  methods: {
    /** 查询GeneralAffairs签核人员列表 */
    getList() {
      debugger;
      this.loading = true;
      listGeneralAffairslist(this.queryParams).then(response => {
        this.GeneralAffairslistList = response.rows;
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.generalaffairslistid)
    ...
    handleUpdate(row) {
      debugger;
      this.reset();
      const generalaffairslistid = row.generalaffairslistid || this.ids
      getGeneralAffairslist(generalaffairslistid).then(response => {
    ...
    submitForm() {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.generalaffairslistid != null) {
            updateGeneralAffairslist(this.form).then(response => {
            ...
            addGeneralAffairslist(this.form).then(response => {
              if (response.code === 200) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const generalaffairslistids = row.generalaffairslistid || this.ids;
      this.$confirm('是否确认删除签核人员编号为"' + generalaffairslistids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delGeneralAffairslist(generalaffairslistids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有签核人员数据项?', "警告", {
          ...
          return exportGeneralAffairslist(queryParams);
        }).then(response => {
