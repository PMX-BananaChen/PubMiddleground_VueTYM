<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <!-- <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:Unitsupervisor:add']"
        >新增</el-button>
      </el-col> -->
      <!-- <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:Unitsupervisor:edit']"
        >修改</el-button>
      </el-col> -->
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:Unitsupervisor:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="info"
                   ...
                   size="mini"
                   v-hasPermi="['system:Unitsupervisor:import']"
        ...
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:Unitsupervisor:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="UnitsupervisorList" @selection-change="handleSelectionChange">
...
<script>
import { listUnitsupervisor, getUnitsupervisor, delUnitsupervisor, addUnitsupervisor, updateUnitsupervisor, exportUnitsupervisor }
from "@/api/goodpass/Unitsupervisor";
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + '/system/Unitsupervisor/importData'
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listUnitsupervisor(this.queryParams).then(response => {
        this.UnitsupervisorList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getUnitsupervisor(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateUnitsupervisor(this.form).then(response => {
            ...
            addUnitsupervisor(this.form).then(response => {
              if (response.code === 200) {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有直屬權責主管数据项?', "警告", {
          ...
          return exportUnitsupervisor(queryParams);
        }).then(response => {
