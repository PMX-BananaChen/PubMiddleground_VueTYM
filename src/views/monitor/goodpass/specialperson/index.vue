<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:SpecialPerson:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:SpecialPerson:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:SpecialPerson:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:SpecialPerson:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="SpecialPersonList" @selection-change="handleSelectionChange">
...
<script>
import { listSpecialPerson, getSpecialPerson, delSpecialPerson, addSpecialPerson, updateSpecialPerson, exportSpecialPerson }
 from "@/api/goodpass/SpecialPerson";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSpecialPerson(this.queryParams).then(response => {
        this.SpecialPersonList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getSpecialPerson(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateSpecialPerson(this.form).then(response => {
            ...
            addSpecialPerson(this.form).then(response => {
              if (response.code === 200) {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有G4人员数据项?', "警告", {
          ...
          return exportSpecialPerson(queryParams);
        }).then(response => {
