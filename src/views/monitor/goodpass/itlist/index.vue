<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:ITlist:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:ITlist:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="ITlistList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="主键" align="center" prop="itlistid" />
...
<script>
import { listITlist, getITlist, delITlist, addITlist, updateITlist, exportITlist }
from "@/api/goodpass/ITlist";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listITlist(this.queryParams).then(response => {
        this.ITlistList = response.rows;
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.itlistid)
    ...
    handleUpdate(row) {
      debugger;
      this.reset();
      const itlistid = row.itlistid || this.ids
      getITlist(itlistid).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.itlistid != null) {
            updateITlist(this.form).then(response => {
            ...
            addITlist(this.form).then(response => {
              if (response.code === 200) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const itlistids = row.itlistid || this.ids;
      this.$confirm('是否确认删除HR签核人员编号为"' + itlistids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delITlist(itlistids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有HR签核人员数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportITlist(queryParams);
        }).then(response => {
