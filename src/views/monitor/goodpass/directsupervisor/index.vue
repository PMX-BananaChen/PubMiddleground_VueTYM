<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <!-- <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:DirectSupervisor:add']"
        >新增</el-button>
      </el-col> -->
      <!-- <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:DirectSupervisor:edit']"
        >修改</el-button>
      </el-col> -->
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:DirectSupervisor:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="info"
                   ...
                   size="mini"
                   v-hasPermi="['system:DirectSupervisor:import']"
        ...
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:DirectSupervisor:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="DirectSupervisorList" @selection-change="handleSelectionChange">
...
<script>
import { listDirectSupervisor, getDirectSupervisor, delDirectSupervisor, addDirectSupervisor, updateDirectSupervisor, exportDirectSupervisor }
from "@/api/goodpass/DirectSupervisor";
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + '/system/DirectSupervisor/importData'
      },
      areaidOps:null,
    };
  },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listDirectSupervisor(this.queryParams).then(response => {
        this.DirectSupervisorList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getDirectSupervisor(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateDirectSupervisor(this.form).then(response => {
            ...
            addDirectSupervisor(this.form).then(response => {
              if (response.code === 200) {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除直屬權責主管编号为"' + ids + '"的数据项?', "警告", {
          ...
          return delDirectSupervisor(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有直屬權責主管数据项?', "警告", {
          ...
          return exportDirectSupervisor(queryParams);
        }).then(response => {
